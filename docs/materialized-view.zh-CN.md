...
这组边界是代码中人为设置的运行条件，不是多版本 E2E 得出的兼容性结论。
目前没有通过真实集群测试证明 2.1.5 是准确最低版本，也没有证明 3.0.0
一定不兼容。因此本文不把 Gate 放行范围称为“支持版本范围”；除上述已实测
版本外，投入生产前需要在对应 Doris 版本上运行 Functional Test。
